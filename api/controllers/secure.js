...
//   try {
//     const decoded = jwt.verify(token, JWTSECRET); // ✅ konsisten
...
async function authenticateTokenWithSession(req, res, next) {
  const authHeader = req.headers['authorization'];
  const tokenlogin = authHeader?.split(' ')[1];
  const tokenSignature = tokenlogin?.split('.')[2];

  if (!tokenlogin || !tokenSignature) {
    return res.status(401).json({ message: 'Token tidak tersedia' });
  }

  try {
    const decoded = jwt.verify(tokenlogin, JWTSECRET); // <- ini perlu
